...
Figures generated
-----------------
  fig1_convergence_D{ref}.{fmt}  Convergence curves at D=ref, all functions
  fig2_convergence_{func}.{fmt}  Convergence for one function, all D
...
-----
  ...
  python experiments/plot_results.py --input results_neorl/results.json
...
class Dataset:
    ...
    def __init__(self, data: dict) -> None:
        ...
        _cec17_class = {
            ...
            **{"f{}".format(i): "Multimodal" for i in range(4, 11)},
            **{"f{}".format(i): "Hybrid"     for i in range(11, 21)},
            **{"f{}".format(i): "Composition" for i in range(21, 31)},
        }
        _simple = {
        ...
        for fname in self.func_names:
            if fname in _simple:
                self.label[fname] = _simple[fname]
            else:
                cls = _cec17_class.get(fname, "")
                self.label[fname] = "{}  [{}]".format(fname.upper(), cls)
    ...
    def convergence(
        ...
        for r in runs:
            ...
            matrix.append(np.maximum(row, 1e-300))
...
def _save(fig, path: Path, fmt: str) -> None:
    p = path.with_suffix(".{}".format(fmt))
    fig.savefig(p)
    plt.close(fig)
    print("  Saved: {}".format(p))


# ─── Figure 1: All functions at D_ref ────────────────────────────────────────
...
def fig1_convergence_all_funcs(ds: Dataset, d_ref: int, out: Path, fmt: str) -> None:
    ...
    n     = len(funcs)
    if n == 0:
        print("  [skip fig1] No data at D={}".format(d_ref))
        return
    ...
    fig.suptitle(
        "DFO Convergence  —  D={},  N=50,  MaxFEs={:,},  30 runs\n"
        "Median error +/- IQR band  (log scale)".format(d_ref, 10000 * d_ref),
        fontsize=11, y=1.01,
    )
    ...
    fig.tight_layout()
    _save(fig, out / "fig1_convergence_D{}".format(d_ref), fmt)


# ─── Figure 2: One function, all D ───────────────────────────────────────────

def fig2_convergence_one_func(ds: Dataset, fname: str, out: Path, fmt: str) -> None:
    dims = [D for D in ds.dims if len(ds.errors(fname, D)) > 0]
    if not dims:
        print("  [skip fig2] No data for {}".format(fname))
        return
    ...
    for D in dims:
        ...
        ax1.semilogy(frac, med, color=c, lw=2.0, label="D={}".format(D))

    ax1.set_xlabel("FEs / MaxFEs (fraction of budget)")
    ax1.set_ylabel(r"$|f - f^*|$  (log scale)")
    ax1.set_title("{} — convergence by D\n(normalised x-axis)".format(ds.label[fname]))
    ...
    ax2.set_title("{} — final error distribution\n30 runs, log scale".format(ds.label[fname]))

    fig.suptitle("{} — N=50, 30 runs, f*={:.0f}".format(
        ds.label[fname], ds.f_star[fname]), fontsize=11)
    fig.tight_layout()
    _save(fig, out / "fig2_convergence_{}".format(fname), fmt)
...
def fig3_heatmap(ds: Dataset, out: Path, fmt: str) -> None:
    ...
    ax.set_xticklabels(["D={}".format(D) for D in dims])
    ax.set_yticks(range(len(funcs)))
    ax.set_yticklabels([ds.label[f] for f in funcs], fontsize=8)
    ax.set_title("Mean final error (log10) — DFO, N=50, 30 runs\n"
                 "Green = small error  ·  Red = large error", pad=8)

    for i in range(len(funcs)):
        for j in range(len(dims)):
            if annot[i][j] and not np.isnan(mat[i, j]):
                nv = (mat[i, j] - vmin) / (vmax - vmin + 1e-9)
...
def fig4_boxplots(ds: Dataset, out: Path, fmt: str) -> None:
    ...
    for ax, fname in zip(axes_flat, funcs):
        vdims  = [D for D in dims if len(ds.errors(fname, D)) > 0]
        data   = [np.maximum(ds.errors(fname, D), 1e-300) for D in vdims]
        if not data:
            ax.set_visible(False)
            continue
        ...
        for patch, D in zip(bp["boxes"], vdims):
            patch.set_facecolor(DIM_COLORS.get(D, "#888"))
            patch.set_alpha(0.6)
        ax.set_yscale("log")
        ax.set_xticks(range(1, len(vdims) + 1))
        ax.set_xticklabels(["D={}".format(D) for D in vdims], fontsize=8)
        ax.set_ylabel(r"$|f - f^*|$", fontsize=8)
        ax.set_title(ds.label[fname], fontsize=8, pad=4)
        ax.tick_params(labelsize=7)
    ...
    fig.suptitle("DFO — Final error distribution by dimension (30 runs, log scale)",
                 fontsize=11)
    fig.tight_layout()
...
def fig5_improvement_rate(ds: Dataset, out: Path, fmt: str) -> None:
    ...
    for ax, fname in zip(axes_flat, funcs):
        for D in dims:
            ...
            for r in runs:
                ...
                if len(fes_r) < 3:
                    continue
                w    = max(1, int(WINDOW_FRAC * max(fes_r)))
                ir_v = []
            ...
            ax.plot(ref_fes[:min_len], median,
                    color=DIM_COLORS.get(D, "#888"), lw=1.6, label="D={}".format(D))
    ...
    fig.suptitle(
        ...
        "Rolling window = {}% of MaxFEs  ·  "
        "dashed: Phase I (0.30) / stagnation (0.05)".format(int(WINDOW_FRAC * 100)),
        fontsize=11,
    )
    fig.tight_layout()
...
def fig6_collapse_profile(ds: Dataset, out: Path, fmt: str) -> None:
    ...
    for fname, color in zip(ds.func_names, _PALETTE_29):
        means, medians, vdims = [], [], []
        for D in ds.dims:
            errs = ds.errors(fname, D)
            if len(errs) == 0:
    ...
    for ax, title in [(ax1, "Mean"), (ax2, "Median")]:
        ...
        ax.set_title("{} error vs D — DFO, N=50".format(title))
        ax.set_xticks(ds.dims)
        if len(ds.func_names) <= 12:
...
def table1_summary(ds: Dataset, out: Path) -> None:
    ...
    for fname in ds.func_names:
        ...
        for D in ds.dims:
            errs = ds.errors(fname, D)
            if len(errs) == 0:
                rows.append([ds.label[fname], "{:.0f}".format(fstar), D] + ["N/A"] * 6)
                continue
            mu  = float(np.mean(errs))
            ...
            rows.append([
                ds.label[fname], "{:.0f}".format(fstar), D,
                "{:.4e}".format(mu), "{:.4e}".format(sd), "{:.4e}".format(med),
                "{:.4e}".format(float(np.min(errs))),
                "{:.4e}".format(float(np.max(errs))),
                "{:.1f}".format(sr),
            ])
    ...
    with open(tex_path, "w") as f:
        ...
        f.write("\\caption{DFO results ($|f_{best}-f^*|$, $N=50$, "
                "MaxFEs$=10^4D$, 30 runs)}\n")
        f.write("\\label{tab:dfo}\n\\small\n")
        f.write("\\begin{tabular}{llrcccccr}\n\\toprule\n")
        f.write("Func & $f^*$ & $D$ & Mean & Std & Median & "
                "Best & Worst & SR\\%\\\\\n\\midrule\n")
        prev = None
        for row in rows:
...
def table2_wilcoxon(ds: Dataset, out: Path) -> None:
    ...
    for fname in ds.func_names:
        for i, D1 in enumerate(dims):
            for j, D2 in enumerate(dims):
                ...
                sym  = ("+" if np.median(e1) > np.median(e2) else "-") if p < 0.05 else "approx"
    ...
    SYM_TEX  = {"+": "$+$", "-": "$-$", "approx": r"$\approx$"}
    with open(tex_path, "w") as f:
        f.write("% Table 2: Wilcoxon rank-sum pairwise comparisons\n\n")
        f.write("\\begin{table}[htbp]\\centering\n")
        f.write("\\caption{Wilcoxon pairwise D comparison ($\\alpha=0.05$, two-sided). "
                "$+$/$-$: row D significantly worse/better.}\n")
        f.write("\\label{tab:wilcoxon}\n")
        f.write("\\begin{tabular}{l" + "c" * len(dims) + "}\n\\toprule\n")
        f.write("& " + " & ".join("$D={}$".format(D) for D in dims) + " \\\\\n\\midrule\n")
        for fname in ds.func_names:
            f.write("\\multicolumn{{{}}}{{l}}{{\\textit{{{}}}}} \\\\\n".format(
                len(dims) + 1, ds.label[fname]))
            for D1 in dims:
                cells = []
                for D2 in dims:
                    if D1 == D2:
                    ...
                    for (fn, d1, d2, p, s) in tex_store:
                        if fn == fname and d1 == D1 and d2 == D2:
                            sym = SYM_TEX.get(s, s); break
                        if fn == fname and d1 == D2 and d2 == D1:
                            inv = {"+" : "-", "-": "+", "approx": "approx"}
                            sym = SYM_TEX.get(inv.get(s, s), s); break
                    cells.append(sym)
                f.write("  $D={}$ & ".format(D1) + " & ".join(cells) + " \\\\\n")
        f.write("\\bottomrule\n\\end{tabular}\n\\end{table}\n")
    print("  Saved: {}".format(tex_path))
...
def main(args: argparse.Namespace) -> None:
    rpath = Path(args.input)
    if not rpath.exists():
        print("[ERROR] Not found: {}".format(rpath), file=sys.stderr)
    ...
    print("Input  : {}".format(rpath))
    print("Output : {}/  (format: {})".format(out, fmt))
    print("Funcs  : {}".format(ds.func_names))
    print("Dims   : {}".format(ds.dims))
    print()
    ...
    print("\nAll outputs in: {}/".format(out))


def parse_cli() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Generate plots and tables from DFO CEC2017 results",
    ...
    p.add_argument("--d-ref",    type=int, default=30,
                   help="Reference D for fig1")
    p.add_argument("--func-ref", default="f1",
                   help="Reference function for fig2 (e.g. f1, sphere)")
    return p.parse_args()
